...
behind most reports of a setting that "does nothing".
...
are unlit under a default profile. See [Palette](palette.md#torch-requires-generatelighting).
...
A profile that wants undamaged buildings has to zero both.
...
losing foliage. See [Profile](profile.md).
...
[What changed in 7.5](../versions/7-5.md).
...
world on 7.4.12. See [Building](building.md#how-floor-and-cellar-counts-are-decided).
...
the set defines with a real block.

Checked in a world on 7.4.12. See [Palette](palette.md#collisions-and-merge-order).

## Chains where every link must hold

### A chest is filled only if four things agree

| Gate | Default | Effect when it fails |
|---|---|---|
...
Checked in a world on 7.4.12. See [Palette](palette.md#why-a-chest-generates-empty).

### A profile reaches a datapack through four names

`dimensionsWithProfiles` names a profile, the profile names a `worldStyle`, the
world style names city styles, and a city style names buildings.

A wrong name at the first three is not equal in consequence:

| Wrong name in | Result |
|---|---|
...
null, so the first chunk that asks about the world outside a sphere throws.

On those landscape types nothing catches it, because `LostCitySphereFeature` has no
`try`. Checked in a world on 7.4.12.
...
Checked in a world on 7.4.12. See [Predefined City](predefined.md).
...
- [Profile Reference](profile.md) for the keys themselves
