...
| `PRF-8`{#prf-8} | The five `client` keys do nothing on 7.4.12 | Confirmed, code and game. `javap` over every class in `lostcities-1.20-7.4.12.jar` finds `FOG_RED` and `FOG_BLUE` referenced only by `ProfileSetup` and `LostCityProfile`, where they are declared, written from the two config load paths, and read once by a copy constructor. There is **no subscriber to `ViewportEvent`, `RenderFog` or `ComputeFogColor`, and no use of `FogRenderer`, anywhere in the jar**; `ClientEventHandlers` draws the Cities button and nothing else. Then game tested at a client on 2026-08-28: same seed, same position (-50, 131, -84), two profiles differing only in the `client` section, one with `fogRed` 1.0, `fogGreen` and `fogBlue` 0, `fogDensity` 0.9 and `horizon` 200. Sky, fog colour, fog density and horizon identical in both, while the city generated from `lostcities:standard` at `cityChance` 0.4 in each, so the profile was read and the five keys changed nothing. Values chosen to be extreme so that "did nothing" could not be mistaken for "did something small". Read from 7.4.12 only; the later versions have not been swept. |
