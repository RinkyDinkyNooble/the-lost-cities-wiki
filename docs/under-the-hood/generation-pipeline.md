...
- A part placed rotated or mirrored rotates only the blocks in the [`lostcities:rotatable` tag](../reference/palette.md#rotation-and-the-lostcitiesrotatable-tag), plus rails, which always rotate. Covered in full on the Palette page. [code review](../examples/claim-tests.md#rot-1){.v .v-c}
