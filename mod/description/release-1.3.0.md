...
rather than the first one repeated, and a palette entry's `tag` is carried both ways,
so command blocks keep their commands and chests keep their loot tables.
`/lcdev import <style> run` pastes command blocks live, so they fire and resolve into
whatever they place; without it they arrive holding their command but unable to run.
