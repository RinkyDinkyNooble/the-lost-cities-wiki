...
| `mod/tools/check-config.py` | A config toggle changes what the mod does, in both directions. `fixFullStreetShape` off draws none of the `full` street shape and on draws 1504 of it, which is the first time a repair's code has run under a check; `validateOnLoad` reports or stays silent; a commented asset loads or stops the server starting; and Lost Cities generates from a `.json5` world style or does not |
